...
## `Package Control`
### Installation
The manual is [Here](https://packagecontrol.io/installation#Manual).

### Install behind proxy
Sublime Textの`"View" -> "Show Console"`でコンソールを表示した後、以下のワンライナーを入力。
ただし、ワンライナーの次の部分は自分のものに置き換えること。
- [username]
- [password]
- [proxy_server]
- [port]
...
hash値(`h = ...`の部分)が違う場合は、コンソールログに正しいhashが表示されるので差し替えれば通るはず。
そのときは、`"got A instead of B"`みたいなエラーが出てくる。

### Proxy setting for Package Control

Package Controlにプロキシ情報を書いておく。
`"Preferences" -> "Package Settings" -> "Package Control" -> "Settings - User"`に次を追加。

```js
{
    "http_proxy": "http: http://[username]:[passwd]@[proxy_server]:[port]",
    "https_proxy": "http: http://[username]:[passwd]@[proxy_server]:[port]",
...
## Preferences
#### 不可視文字の可視化とソフトタブ
`Preferences > Settings - User` を次のように書き換える
```js
{
    // Font Setting
    // "font_face": "HackGen Console Regular",
    // "font_face": "Firge Regular",
    // "font_face": "FirgeNerd Regular",
    // "font_face": "Fira Code Regular",
    "font_face": "Cica-Regular",
    "font_size": 13,
    // インデントの幅
    "tab_size": 4,
    // タブをスペースに変換
    ...
    "color_scheme": "Dark+.sublime-color-scheme",
    "index_files": true,
}
```
...
### Package固有のカスタマイズ・エラー対処
#### MarkdownTOC
MarkdownTOCのsetting-userに以下を加筆
...
#### OmniMarkupPreviewer
HTML/Markdownプレビューが`404 Not Found`になるときの対処法：
